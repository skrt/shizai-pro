...
| Drawer | drawer.html | 右スライドDrawer（メッセージ送受信） |
...
### アイコン
- **必ず Lucide アイコンを使用すること**（https://lucide.dev/icons/）
- Rails では importmap + Stimulus、または SVG 直接埋め込みで利用
- カタログでは `icons.svg` スプライトシートで管理
- 他のアイコンライブラリ（Heroicons, Font Awesome 等）は使わない
