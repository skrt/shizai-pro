...
画面はユーザー種別・機能単位で **Section** にまとめる。Section 名は役割がわかる名前にする（例: `デマンド`, `社内メンバー`, `オペレーター`）。

### Section 内のレイアウト

```
Section
├── Row 1（上段）: メイン画面 → ダイアログ画面（横並び）
└── Row 2（下段）: トースト付き画面（横並び）
```

- Section 内の開始位置: `x:100, y:100`
- フレーム間の横ギャップ: **90px**（`1440 + 90 = 1530` ごと）
- Row 間の縦ギャップ: **100px**
- 上段はユーザーの操作順に左→右で並べる（メイン画面 → 確認ダイアログ → 結果画面）
- 下段はフィードバック状態（トースト表示）をまとめる

### 新しい画面を追加するとき

1. 該当 Section を確認（なければ Section を作成）
2. Section 内の既存フレーム配置を確認
3. 適切な Row・位置にフレームを追加
4. Section のサイズを必要に応じて拡張
